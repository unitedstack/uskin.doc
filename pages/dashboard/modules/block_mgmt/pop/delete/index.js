...
const handleVolumeWithClient = (err) => {
  let message = JSON.parse(err.response).data.message;
  if(message && /Allocated to/.test(message)){
    return  __.volume_related_client + message.match(/(Allocated to:\s)(.*)/)[2] + __.volume_related_client_delete;
  } else {
  ...
  let props = {
    ...
    onConfirm: function(values, cb) {
      request.deleteVolume(obj.id).then(res => {
        ...
        cb(false, handleVolumeWithClient(err));
      });
    },
