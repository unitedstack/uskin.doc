...
class Monitor extends React.Component {
  ...
  getMonitorData(chartKey, selectedKey) {
    ...
    switch (chartKey) {
      case 'iops':
        request.getIOPSMonitorData(osdId, selectedKey).then(res => {
        ...
        break;
      case 'bandwidth':
        request.getBandwidthMonitorData(osdId, selectedKey).then(res => {
        ...
        break;
      case 'io_wait_time':
        request.getWaitTimeMonitorData(osdId, selectedKey).then(res => {
          copy.data.forEach((item) => {
            item.data = res[item.key].map(resItem => {
              return [
                resItem[0],
  ...
  render() {
    ...
    return (
      <div className="uds-osd-detail-monitor-wrapper">
