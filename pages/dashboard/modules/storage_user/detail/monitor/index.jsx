...
class Monitor extends React.Component {
  ...
  getMonitorData(chartKey, selectedKey) {
    ...
    switch (chartKey) {
      case 'upload_bandwidth':
        request.getUploadBandwidth(user, selectedKey).then(res => {
      ...
      case 'download_bandwidth':
        request.getDownloadBandwidth(user, selectedKey).then(res => {
          let unitMax = unit.unitFix(res);
        ...
        break;
      case 'upload_time':
        request.getUploadtime(user, selectedKey).then(res => {
          copy.data.forEach(item => {
            item.data = res.map(resItem => {
              return [
                resItem[0],
        ...
        break;
      case 'download_time':
        request.getDownloadtime(user, selectedKey).then(res => {
          copy.data.forEach(item => {
            item.data = res.map(resItem => {
              return [
                resItem[0],
  ...
  render() {
    ...
    return (
      <div className="storage-user-detail-monitor-wrapper">
        <ul>
          {
